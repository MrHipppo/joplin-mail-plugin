...
function randomIndexes(arrayLength, count) {
    const arr = [];
...
// should be created.
function populateDatabase(db, options = null) {
    return __awaiter(this, void 0, void 0, function* () {
        options = Object.assign({ folderCount: 0, noteCount: 0, tagCount: 0, tagsPerNote: 0, clearDatabase: false, rootFolderCount: 0, subFolderDepth: 0 }, options);
        if (options.clearDatabase)
            yield db.clearForTesting();
        const createdFolderIds = [];
        const createdNoteIds = [];
        const createdTagIds = [];
        const createdFolderDepths = {};
        const folderDepthToId = {};
        ...
        for (let i = 0; i < options.folderCount; i++) {
            ...
            if (options.rootFolderCount && rootFolderCount >= options.rootFolderCount)
                isRoot = false;
            let depth = 0;
            if (!isRoot) {
                let possibleFolderIds = [];
                if (options.subFolderDepth) {
                    for (let i = 0; i < options.subFolderDepth; i++) {
                        if (folderDepthToId[i])
                            possibleFolderIds = possibleFolderIds.concat(folderDepthToId[i]);
                    }
                }
                else {
            ...
            const savedFolder = yield Folder_1.default.save(folder);
            createdFolderIds.push(savedFolder.id);
            createdFolderDepths[savedFolder.id] = depth;
            if (!folderDepthToId[depth])
                folderDepthToId[depth] = [];
            folderDepthToId[depth].push(savedFolder.id);
            // eslint-disable-next-line no-console
            if (!options.silent)
                console.info(`Folders: ${i} / ${options.folderCount}`);
        }
        let tagBatch = [];
        for (let i = 0; i < options.tagCount; i++) {
            const tagTitle = randomElement(wordList); // `tag${i}`
            // eslint-disable-next-line promise/prefer-await-to-then -- Old code before rule was applied
            tagBatch.push(Tag_1.default.save({ title: tagTitle }, { dispatchUpdateAction: false }).then((savedTag) => {
                createdTagIds.push(savedTag.id);
                // eslint-disable-next-line no-console
                if (!options.silent)
                    console.info(`Tags: ${i} / ${options.tagCount}`);
            }));
            if (tagBatch.length > 1000) {
                yield Promise.all(tagBatch);
                tagBatch = [];
            }
        }
        if (tagBatch.length) {
        ...
        if (options.tagsPerNote) {
            ...
            for (const noteId of createdNoteIds) {
                ...
                noteTagBatch.push(Tag_1.default.setNoteTagsByIds(noteId, tagIds));
                if (noteTagBatch.length > 1000) {
                    yield Promise.all(noteTagBatch);
                    noteTagBatch = [];
                }
            }
