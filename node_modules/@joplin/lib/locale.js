...
const parsePluralForm = (form) => {
    const pluralFormRegex = /^(\s*nplurals\s*=\s*[0-9]+\s*;\s*plural\s*=\s*(?:\s|[-?|&=!<>+*/%:;a-zA-Z0-9_()])+)$/m;
    if (!pluralFormRegex.test(form))
        throw new Error(`Plural-Forms is invalid: ${form}`);
    if (!/;\s*$/.test(form)) {
        form += ';';
    }
    const code = [
...
exports.parsePluralForm = parsePluralForm;
const getPluralFunction = (lang) => {
    if (!(lang in pluralFunctions_)) {
        ...
        if (!stats.pluralForms) {
            ...
            pluralFunctions_[lang] = (0, exports.parsePluralForm)(stats.pluralForms);
        }
    }
